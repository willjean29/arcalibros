...
const ItemPreviewQuestionBank: React.FC<IProps> = ({ excercise, index }) => {
  const dispatch = useDispatch();
  const newExamExcercises = useSelector(
  ...
  const handleAddExcercise = (e: MouseEvent) => {
    e.preventDefault();
    if (!added) {
      dispatch(examAddExcercise(excercise));
      setAdded(true);
    }
  };
  useEffect(() => {
    if (
      newExamExcercises.find(
...
export default ItemPreviewQuestionBank;
