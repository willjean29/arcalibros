...
import { addClassroomComment } from "../../store/classroom/classroom.actions";
...
const PostField: React.FC<PostFieldProps> = ({ imgUser, classroomId }) => {
  ...
  const handleClassroomComment = async (e: FormEvent) => {
    e.preventDefault();
    ...
    if (newFile === null) {
      ...
      dispatch(addClassroomComment(classroomCommentDto, classroomId));
      setLink("");
    } else {
      ...
      try {
        ...
        const res = await axios.post(
          classroomsURL.concat(`upload/${hour}&${classroomId}`),
          formData,
          {
            headers: {
        ...
        if (res.data.file) {
          const classroomCommentFileDto = {
            author: _id as string,
            name: firstName.concat(" " + lastName),
            type,
            comment,
            ...(link ? { link: finalContent } : undefined),
            file: `<a href="https://meet.arcavirtual.net/classrooms/download/${classroomId}&${hour}${res.data.file}" target="_blank">${newFile.name}</a>`,
            date: moment().locale("es").format("LLL"),
            profileImg: profileImg !== "default" ? profileImg : "default",
          };
          dispatch(addClassroomComment(classroomCommentFileDto, classroomId));
          setComment("");
          setLoading(null);
          setNewFile(null);
          setLink("");
  ...
  return loading === true ? (
    ...
    <div className="field-post">
      ...
      <form className="input-field" onSubmit={handleClassroomComment}>
